...
@router.get(
    "/owner/{restaurant_id}/analytics",
    response_model=OwnerAnalyticsReport
)
async def get_owner_analytics(
    ...
    try:
        ...
        return await dashboard_service.get_owner_analytics(
            restaurant_id=restaurant_id,
            from_date=from_datetime,
            to_date=to_datetime,
            authorization_header=authorization,
...
@router.get("/owner/{restaurant_id}/analytics/export")
async def export_owner_analytics(
    restaurant_id: str,
    from_date: date | None = Query(default=None, alias="from"),
    to_date: date | None = Query(default=None, alias="to"),
    branch_id: str | None = Query(default=None, alias="branchId"),
    ...
    try:
        ...
        report = await dashboard_service.get_owner_analytics(
            restaurant_id=restaurant_id,
            from_date=from_datetime,
            to_date=to_datetime,
            authorization_header=authorization,
        ...
        excel_file = excel_export_service.build_owner_analytics_workbook(
            report=report,
            from_date=resolved_from_date,
            to_date=resolved_to_date
        )
        ...
        filename = (
            f"tap2eat-reportes-{restaurant_id}-"
            f"{resolved_from_date.isoformat()}-"
            f"{resolved_to_date.isoformat()}.xlsx"
        )
